...
require_once( __DIR__ . '/../../lib/summarize_trades_multimarket.class.php');
...
try {
    ...
    $basecurrency = $market ? primary_market::determine_primary_market_symbol_from_market($market) :
                              primary_market::get_normalized_primary_market_symbol($basecurrency);
	...
	$summarizer = new summarize_trades_multimarket($network);
	
	$criteria = ['datetime_from' => $start,
				 'datetime_to' => $end,
				 'integeramounts' => false,
				 'fields' => ['period_start', 'volume', 'num_trades'],
				 'sort' => 'asc',
				 'fillgaps' => $fillgaps,
				];
